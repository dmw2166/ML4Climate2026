...
**Parameter emulation.** The inputs are the simulator's own tunable parameters — cloud,
convection and microphysics settings — and the output is the climate the model produces. This is
...
**Scenario emulation.** The inputs are forcings — emissions or concentrations of CO$_2$, methane,
aerosols — and the output is the climate response. The purpose is **scenario exploration**: the
...
of the scientific value is in **spatial** output — where it warms, where precipitation changes,
where the extremes shift — and that is a much larger output space. A single 96×144 field is
...
the simulator's behaviour changes qualitatively somewhere — a circulation regime shift, an ice
sheet collapse, a tipping point — nothing in the training data anticipates it, and the emulator
